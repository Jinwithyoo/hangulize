...
'''
CODE_SEP = '\n        '
ASSERTION_TEMPLATE = "assert u'{want}' == self.hangulize(u'{word}')"
...
class gen_test(Command):
    ...
    def run(self):
        if not self.url:
            self.url = input('Rules URL(http://korean.go.kr/...): ')
        if not self.name:
            self.name = input('Language Name(ex. Italian): ')
        if not self.locale:
            self.locale = input('Locale Code(ex. it): ')
        path = os.path.join('tests', '%s.dist.py' % self.locale)
        with open(path, 'w') as out:
            print('generating test suite...', end=' ')
            print(generate_testsuite(self.url,
                                           self.name,
...
def generate_testsuite(url, name, locale):
    ...
    html = ''.join(urllib.request.urlopen(url).readlines())
