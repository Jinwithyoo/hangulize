...
class REPLHandler(logging.StreamHandler):
    ...
    def handle(self, record):
        ...
        # keywords
        maxlen = max([len(x) for x in list(self.color_map.keys())])
        def deco(color_name):
            def replace(m):
                pad = ' ' * (maxlen - len(m.group(1)))
                return color(m.group(1), color_name) + pad
            return replace
        for keyword, color_name in list(self.color_map.items()):
...
class repl(Command):
    ...
    def run(self):
        ...
        def _repl():
            while True:
                lang = self.lang or input(color('Lang: ', 'magenta'))
                try:
                    lang = get_lang(lang)
